...
class sp_hd_handler():
    ...
    def handle_transdtl(self, transdtl, fid):
        ...
        log_str += tmp_str
        tmp_str = u"lastlimamt=[{0:02X}][{1:02x}][{2:02x}]\n".format(ord(lastlimamt[0]), ord(lastlimamt[1]),ord(lastlimamt[2]))
        log_str += tmp_str
        tmp_str = u"lasttransflag={0:02x}\nlasttermno={1:02x}{2:02x}{3:02x}{4:02x}{5:02x}{6:02x}\n".format(lasttransflag,
            ord(lasttermno[0]),ord(lasttermno[1]), ord(lasttermno[2]), ord(lasttermno[3]), ord(lasttermno[4]), ord(lasttermno[5]))
        log_str += tmp_str
        tmp_str = u"lastdatetime={0:02x}{1:02x}{2:02x}{3:02x}{4:02x}{5:02x}\n".format(ord(lastdatetime[0]),ord(lastdatetime[1]),
            ord(lastdatetime[2]), ord(lastdatetime[3]),ord(lastdatetime[4]),ord(lastdatetime[5]))
        log_str += tmp_str
        tmp_str = u"cardbefbal={0}\ncardbefcnt={1}\namount={2}\n".format(cardbefbal, cardbefcnt, amount)
        log_str += tmp_str
        tmp_str = u"extraamount=[{0:02x}][{1:02x}][{2:02x}]\n".format(ord(extraamount[0]), ord(extraamount[1]),ord(extraamount[2]))
        log_str += tmp_str
        tmp_str = u"transdatetime={0:02x}{1:02x}{2:02x}{3:02x}{4:02x}{5:02x}\n".format(ord(transdatetime[0]),ord(transdatetime[1]),
            ord(transdatetime[2]),ord(transdatetime[3]),ord(transdatetime[4]),ord(transdatetime[5]))
        log_str += tmp_str
        tmp_str = u"psamno={0:02x}{1:02x}{2:02x}{3:02x}{4:02x}{5:02x}\n".format(ord(psamno[0]), ord(psamno[1]),ord(psamno[2]),
            ord(psamno[3]), ord(psamno[4]), ord(psamno[5]))
        log_str += tmp_str
        tmp_str = u"tac={0:02x}{1:02x}{2:02x}{3:02x}\ntransflag={4:02x}\nreserve={5:02x}{6:02x}\ncrc={7:02x}{8:02x}\n".format(
            ord(tac[0]), ord(tac[1]), ord(tac[2]), ord(tac[3]),transflag, ord(reserve[0]), ord(reserve[1]), ord(crc[0]), ord(crc[1]))
        log_str += tmp_str
        gloabal_log_handler.sp_transdtl_log(log_str, fid)
        tmp_str = u"amount={4},tac={0:02X}{1:02X}{2:02X}{3:02X}".format(ord(tac[0]), ord(tac[1]), ord(tac[2]), ord(tac[3]), amount)
        #print tmp_str
    ...
    def do_something_with_data(self, s, fid):
        ...
        if m_tcp_unt.cmd_code == 0x01:
            self.handle_transdtl(m_tcp_unt.data, fid)
